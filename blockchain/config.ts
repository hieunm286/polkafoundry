...
const polka: NetworkConfig = {
  id: '11',
  name: 'polka',
  label: 'Polka',
  infuraUrl: `https://rpc-halongbay.polkafoundry.com`,
  infuraUrlWS: ``,
  safeConfirmations: 6,
  otc: contractDesc(otc, '0xe325acB9765b02b8b418199bf9650972299235F4'),
  collaterals: getCollaterals(polkadotAddress),
  tokens: {
    ...getCollateralTokens(polkadotAddress),
    WETH: contractDesc(eth, polkadotAddress['ETH']),
    DAI: contractDesc(erc20, polkadotAddress['MCD_DAI']),
    USDC: contractDesc(erc20, '0x198419c5c340e8De47ce4C0E4711A03664d42CB2'),
    MKR: contractDesc(erc20, polkadotAddress.MCD_GOV),
    CHAI: contractDesc(erc20, '0xb641957b6c29310926110848db2d464c8c3c3f38'),
    // WBTC: contractDesc(erc20, '0xA08d982C2deBa0DbE433a9C6177a219E96CeE656'),
  },
  joins: {
    ...getCollateralJoinContracts(polkadotAddress),
  },
  getCdps: contractDesc(getCdps, polkadotAddress.GET_CDPS),
  mcdOsms: getOsms(polkadotAddress),
  mcdPot: contractDesc(mcdPot, polkadotAddress.MCD_POT),
  mcdJug: contractDesc(mcdJug, polkadotAddress.MCD_JUG),
  mcdEnd: contractDesc(mcdEnd, polkadotAddress.MCD_END),
  mcdSpot: contractDesc(mcdSpot, polkadotAddress.MCD_SPOT),
  mcdCat: contractDesc(mcdCat, polkadotAddress.MCD_CAT),
  dssCdpManager: contractDesc(dssCdpManager, polkadotAddress.CDP_MANAGER),
  otcSupportMethods: contractDesc(otcSupport, '0x303f2bf24d98325479932881657f45567b3e47a8'),
  vat: contractDesc(vat, polkadotAddress.MCD_VAT),
  mcdJoinDai: contractDesc(mcdJoinDai, polkadotAddress.MCD_JOIN_DAI),
  dsProxyRegistry: contractDesc(dsProxyRegistry, polkadotAddress.PROXY_REGISTRY),
  dsProxyFactory: contractDesc(dsProxyFactory, polkadotAddress.PROXY_FACTORY),
  dssProxyActions: contractDesc(dssProxyActions, polkadotAddress.PROXY_ACTIONS),
  etherscan: {
    url: 'https://kovan.etherscan.io',
    apiUrl: 'https://api-kovan.etherscan.io/api',
    apiKey: etherscanAPIKey || '',
...
export const networksById = keyBy([main, kovan, polka, hardhat], 'id')
export const networksByName = keyBy([main, kovan, polka, hardhat], 'name')
